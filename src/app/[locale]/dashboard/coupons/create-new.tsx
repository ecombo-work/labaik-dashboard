...
export default function CreateCoupon({ pre_loader }: { pre_loader: boolean }) {
  ...
  const onSubmit = async (data: FormSchemaType) => {
    try {
      ...
      const couponData = {
        ...restData,
        valid_from: date_range.from ? format(date_range.from, "yyyy-MM-dd") : null,
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Controller
                ...
                control={form.control}
                render={({ field }) => (
                  <FormItem className="col-span-2 flex flex-row items-center justify-between rounded-lg border border-primary h-9 p-2">
            ...
            <DialogFooter>
              ...
              <Button is_loading={isLoading} className="!h-9 !w-30">{t("save")}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
